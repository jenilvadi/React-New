...
export default function Axios() {
  ...
  const addData = async () => {
    if (index == null) {
      ...
      const updateRec = await axios.put(
        `http://localhost:5000/comments/${index}`,
        { name: name, age: Number(age) }
      );
      setName("");
      setAge("");
      setIndex(null);
      const updateData = data.find((item) => {
        item.id == index
          ? {
              ...item,
              name: updateData.data.name,
              age: Number(updateData.data.age),
            }
          : item;
      });
      setData(updateRec);
  ...
  return (
    <>
      <center>
        ...
        <br />
        <br />
        <button id="bt1" onClick={() => addData()}>
          {index ? "Update Data" : "Add Data"}
        </button>{" "}
        <br />
        <br />
        {data &&
